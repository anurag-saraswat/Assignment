...
def find_max(node,lst,final_list ,flag):
    ...
    if flag:
        ...
        detail = {'node':node,'val':0}
        for i in range(len(lst[node])):
            if (i not in final_list) and (1-lst[node][i]) > detail['val']:
                detail['val'] = (1-lst[node][i])
                detail['node'] = i
...
for f in range(t):
    ...
    for h in range(m):
        for z in range(k-1):
            temp=find_max(final_list[-1],diff,final_list,False)
            final_list.append(temp)

        if( (len(final_list) != shops) and (h != m-1)):
            temp = find_max(final_list[-1],diff,final_list,True)
            final_list.append(temp)
