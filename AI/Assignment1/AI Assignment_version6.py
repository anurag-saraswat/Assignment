...
for val in range(shops):
    ...
    for f in range(t):
        for g in range(shops):
            if (f!=0) and(g not in temp_list):
                temp_list.append(g)
                break
        for h in range(m):
            for z in range(k-1):
                temp=find_max(temp_list[-1],sim,temp_list)
                temp_list.append(temp['node'])
            if( (len(final_list) != shops) and (h != m-1)):
                temp_node = {'node':-1,'val':0}
                for pq in range(k,0,-1):
                    temp = find_max(temp_list[-pq],diff,temp_list)
                    if temp_node['val'] < temp['val'] :
                        temp_node['val'] = temp['val']
                        temp_node['node'] = temp['node']
                temp_list.append(temp_node['node'])
